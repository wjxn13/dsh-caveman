...
function apply(ctx) {
  ...
  ctx.inject(["settings"], (settingsCtx) => {
    ...
    console.error("[dsh-caveman] describe immediately after register:", JSON.stringify(settingsCtx.settings.describe().map((d) => d.ns)));
    setTimeout(() => {
      console.error("[dsh-caveman] describe after 3s:", JSON.stringify(settingsCtx.settings.describe().map((d) => d.ns)));
    }, 3e3);
    scope.watch((next) => {
      state = next;
    });
